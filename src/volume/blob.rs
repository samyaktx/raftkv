//! Binary Large Object (Blob) storage with segmented logs and compaction
//! 
//! Storage layout: data_dir/blobs/aa/bb/<percent-encoded-key>
//! where aa/bb are first two bytes of BLAKE3(key)


use crate::common::hash::blob_prefix;
use crate::common::{Result, WalSyncPolicy, blake3_hash, encode_key};
...
impl BlobStore {
    ...
    pub fn stats(&self) -> BlobStats {
        let total_size = self.index.entries().map(|(_, loc)| loc.size).sum();

        BlobStats { 
            total_key: self.index.len(), 
...
mod tests {
    ...
    fn test_blob_stor_basic() {
        ...
        // Delete
        let delete = store.delete("test-key").unwrap();
        assert!(delete);
    ...
    fn test_blob_store_persistence() {
        ...
        // Reopen
        {
            ...
            assert_eq!(store.get("key2").unwrap().unwrap(), b"value1");
        }
    }
}